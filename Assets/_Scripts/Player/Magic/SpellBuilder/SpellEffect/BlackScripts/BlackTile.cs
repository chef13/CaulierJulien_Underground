...
[CreateAssetMenu(menuName = "Magic/Effects/BlackTileEffect")]
public class BlackTileEffectSO : SpellEffectSO
{
...
public override void ApplyEffect(Spell caster, GameObject target = null, int rangelevel = 1, int powerlevel = 1, int durationlevel = 1)
{
    caster.manaCore.UseMana(caster.manaCost);
        ...
        for (int i = caster.targetTile.creatures.Count - 1; i >= 0; i--)
        {
            var creature = caster.targetTile.creatures[i];
            caster.colorHandler.EffectOnCreature(creature);
        }
        ...
        for (int i = 0; i < powerlevel - 1; i++)
        {
            ...
            for (int j = neighborCell.creatures.Count - 1; j >= 0; j--)
            {
                var creature = neighborCell.creatures[j];
                caster.colorHandler.EffectOnCreature(creature);
            }
        }
