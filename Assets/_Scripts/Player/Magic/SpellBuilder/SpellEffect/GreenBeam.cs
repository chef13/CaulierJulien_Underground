...
[CreateAssetMenu(menuName = "Magic/Effects/GreenBeamEffect")]
public class GreenBeamEffectSO : SpellEffectSO
{
    public float beamLength = 8f;

    public float natureChance = 0.01f;

    private static HashSet<TileInfo> natureAffectedTiles = new();
...
public override void ApplyEffect(Spell caster, GameObject target = null, int rangelevel = 1, int powerlevel = 1, int durationlevel = 1)
{
    ...
    for (int i = 0; i <= steps; i++)
    {
        ...
        if (dungeon.dungeonMap.TryGetValue(cell, out TileInfo tile) && tile.isFloor && !tile.isNature)
        {
            float chance = Mathf.Clamp01(natureChance + 0.01f * (powerlevel - 1));
            if (Random.value <= chance)
            {
                tile.isNature = true;
                natureAffectedTiles.Add(tile);
            }
        }
    }
    ...
    tilemapVisualizer.PaintNatureTiles2(dungeon.dungeonMap);
    Debug.Log($"🔵 BlueBeam applied to {natureAffectedTiles.Count} tiles.");
}
